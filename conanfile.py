    ...
    def requirements(self):
        ...
        for req in self.conan_data["requirements"]:
            ...
            if req.startswith("onetbb/") and self.settings.arch == "wasm" and self.settings.os == "Emscripten":
                continue
            # Skip scripta - using local stub instead
            if req.startswith("scripta/"):
                continue
            # Skip libpng - only needed for ARCUS texture loading, which is disabled
            if req.startswith("libpng/"):
                continue
            # Skip onetbb - will use system TBB via CMake find_package
        ...
        if self.options.with_cura_resources:
            for req in self.conan_data["requirements_cura_resources"]:
                self.requires(req)
        # Skip clipper and mapbox-wagyu from ultimaker/stable - will use public versions if available
        # self.requires("clipper/6.4.2@ultimaker/stable")  # Skipped - private package
        ...
        # self.requires("mapbox-wagyu/0.5.0@ultimaker/stable")  # Skipped - private package
